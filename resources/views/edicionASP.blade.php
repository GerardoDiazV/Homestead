@extends('UCN_layout')
@section('title')Editar actividad aprendizaje + servicio
@endsection
@section('pre-body')
...
@section('content')
    ...
    <H1> <center> Editar Actividad de Aprendizaje + Servicio </center> </H1>
    <div class="container">
        <form autocomplete="off" method="POST" action="{{route('asp.update',$actividadASP['id'])}}" enctype="multipart/form-data">
            ...
            <div class="form-group row">
                ...
                <div class="col-sm-3">
                    <input value = "{{$actividadASP['cant_estudiantes']}}" type="number" class="form-control" name= "cant_estudiantes" id="inputEstudiantes"
                        ...
                        <td>
                            <select class="form-control" id="profesorSelect" name= "nombre_profesor[]">
                                ...
                                @foreach($profesores as $profesor)
                                    @if($profesor->nombre_profesor == $profesoresMine->first()->nombre_profesor)
                                        <option selected value="{{$profesor->nombre_profesor}}">{{$profesor->nombre_profesor}}
                                    @else
            ...
            <div class = "form-group row ">
                ...
                <label for="inputEvidencia" class="col-sm-1 col-form-label"></label>
                <div class="col-sm-3 ">
                    ​<figure>
                        <img class = "img-thumbnail" src="{{$evidencias[0]}}">
                ...
                <div class="col-sm-3">
                    <select class="form-control" id="organizacionesSelect" name= "organizacion_id[]">
                        ...
                        @foreach($organizaciones as $socio)
                            @if($socio->id == $organizacionesMine->first()->id)
                                <option selected value="{{$socio->id}}">{{$socio->nombre}}
                            @else
            ...
            <div class = "dinamicInputSpace" id="tableDinamicInput2">
                @foreach($organizacionesMine as $key => $org)
                    @if ($key > 0)
                        <div class= "DinamicInput pt-4">
                            <div class = "form-group row ">
                                ...
                                <div class="col-sm-3 ">
                                    ​<figure>
                                        <img class = "img-thumbnail" src="{{$evidencias[$key]}}">
                                        <figcaption class="figure-caption">Evidencia actual.</figcaption>
                                    </figure>
                                </div>
                            </div>
                            <div class="form-group row">
                                ...
                                <div class="col-sm-3">
                                    <select class="form-control" id="organizacionesSelect" name= "organizacion_id[]">
                                        <option value=""  selected> Seleccione socio comunitario</option>
                                        @foreach($organizaciones as $socio)
                                            @if($socio->id == $organizacionesMine[$key]->id)
                                                <option selected value="{{$socio->id}}">{{$socio->nombre}}
                                            @else
                                                <option value="{{$socio->id}}">{{$socio->nombre}}
                                                    @endif
