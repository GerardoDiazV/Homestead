...
@section('content')
    ...
    @if($actividad_titulacion)
        <ul>
            @foreach($actividad_titulacion as $item)
                <tr>
                    <td class="text-center" id="{{ $item->id }}">{{ $item->id }}</td>
                    <td class="text-left">{{ $item->nombre }}</td>
                    <td class="text-center" width="20%">
                        <div class = "btn-group">
                            <form action="{{route('titulacion.destroy',$item->id)}}" method="POST">
                                {{csrf_field()}}
                            <a class="btn btn-secondary" role="button"href="{{route('titulacion.edit',$item->id)}}" >
                                Editar
                            </a>
